-- SwaDesi App Database Schema Setup (Fixed Version)
...
-- Create cart table (simplified without JSONB unique constraint)
...
DROP TRIGGER IF EXISTS update_bill_uploads_updated_at ON public.bill_uploads;
...
DROP TRIGGER IF EXISTS update_kv_store_updated_at ON public.kv_store;
...
-- Drop existing policies for bill_uploads
DROP POLICY IF EXISTS "Users can view own bill uploads" ON public.bill_uploads;
DROP POLICY IF EXISTS "Users can insert own bill uploads" ON public.bill_uploads;
DROP POLICY IF EXISTS "Users can update own bill uploads" ON public.bill_uploads;
DROP POLICY IF EXISTS "Users can delete own bill uploads" ON public.bill_uploads;
...
-- Drop existing policies for points_activity
DROP POLICY IF EXISTS "Users can view own points activity" ON public.points_activity;
DROP POLICY IF EXISTS "Users can insert own points activity" ON public.points_activity;
...
-- Drop existing policies for cart
...
-- Drop existing policies for kv_store
DROP POLICY IF EXISTS "Authenticated users can access kv_store" ON public.kv_store;
